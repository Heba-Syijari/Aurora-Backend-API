...
import {
  OpenAIAboutContentGenerator,
  OpenAIHeroContentGenerator,
  OpenAITermsAndServiceGenerator,
} from './components';
import { OpenAIFeatureContentGenerator } from './components/openai-feature-content-generator';
import { OpenAIPrivacyPolicyGenerator } from './components/openai-privacy-policy-content-generator';
import { OpenAIChatCompletionService } from './openai-chat-completion.service';

export class OpenAIContentGenerator implements IContentGeneratorContract {
  private readonly chatCompletionService: OpenAIChatCompletionService;
  private readonly heroContentGenerator: OpenAIHeroContentGenerator;
  private readonly featureContentGenerator: OpenAIFeatureContentGenerator;
  private readonly aboutContentGenerator: OpenAIAboutContentGenerator;
  private readonly privacyContentGenerator: OpenAIPrivacyPolicyGenerator;
  private readonly serviceGenerator: OpenAITermsAndServiceGenerator;
  ...
  constructor(
    ...
    this.chatCompletionService = new OpenAIChatCompletionService(openai);
    this.heroContentGenerator = new OpenAIHeroContentGenerator(
      this.chatCompletionService,
      this.model,
    );
    this.featureContentGenerator = new OpenAIFeatureContentGenerator(
      this.chatCompletionService,
      this.model,
    );
    this.aboutContentGenerator = new OpenAIAboutContentGenerator(
      this.chatCompletionService,
      this.model,
    );
    this.privacyContentGenerator = new OpenAIPrivacyPolicyGenerator(
      this.chatCompletionService,
      this.model,
    );
    this.serviceGenerator = new OpenAITermsAndServiceGenerator(
      this.chatCompletionService,
      this.model,
    );
  }
  ...
  public async generateDomainNames(keyword: string): Promise<string[]> {
    ...
    const prompt = [
      `Suggest a list of valid domain names for our islamic website using this keyword '${keyword}'.`,
  ...
  public async enhanceText(text: string): Promise<string> {
    ...
    const prompt = [
      `Enhance the following text: '${text}'`,
      'The content must be Islamic content only',
    ].join('\n');
  ...
  public async generateLogoTexts(keyword: string): Promise<string[]> {
    ...
    const prompt = [
      `Suggest a list of names for our islamic website using this keyword '${keyword}'.`,
  ...
  public async generateText(text: string): Promise<string> {
    ...
    const prompt = [
      `Generate a text using this input text: ${text}`,
      'The content must be Islamic content only',
    ].join('\n');
  ...
  public async generateBlog(dto: PostsContentDto): Promise<BlogContent> {
    ...
    if (!prompt)
      throw new Error('Failed to get the prompt  <OpenAI> : <Blog>.');
    const message = needExternalServiceResponse(
      await this.chatCompletionService.sendMessage(prompt, this.model),
      'Failed to fetch data from external service <OpenAI> : <Blog>.',
    );
    const result = JSON.parse(message);
    if (!result)
      throw new Error(
        'Failed to parse the response as JSON <OpenAI> : <Blog>.',
      );
    return result;
  }
  ...
  public async generateWork(dto: BaseContentDto): Promise<WorkContent> {
    ...
    if (!prompt)
      throw new Error(
        'Failed to get the prompt  <OpenAI> : <<how it works>-One>.',
      );
    const message = needExternalServiceResponse(
      await this.chatCompletionService.sendMessage(prompt, this.model),
      'Failed to fetch data from external service <OpenAI> : <<how it works>-One>.',
    );
    const result = JSON.parse(message);
    if (!result)
      throw new Error(
        'Failed to parse the response as JSON <OpenAI> : <<how it works>-One>.',
      );
