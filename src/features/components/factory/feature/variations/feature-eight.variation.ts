...
import { FeatureEightContent } from 'src/features/content-generator/types';
...
export class FeatureEightVariation implements IComponentVariationFactory {
  constructor(
          ...
          features: [
            {
              title: 'WORKFORCE',
              description:
                '8 Sample text. Click to select the textbox. Click again or double click tostart editing the text. Excepteur sintoccaecat cupidatat non proident.',
              icon: 'tabler:coins',
            },
            {
              title: 'WORKFORCE',
  ...
  private async getContent(input: CreateInput): Promise<FeatureEightContent> {
    try {
      ...
      throw new Error('Failed to generate content for FeatureEightContent.');
    }
  }
}
